...
# diffusion
setwd(paste(getwd(),"/diffusion",sep=""))
source("../SPCL_Stats.R")
...
ggsave(file="scatter.pdf", height=3, width=3, units="in", scale=1.0,
    ggplot(data, aes(x=EST, y=median, colour=COL, shape=COL)) +
        geom_errorbar(aes(ymin=cil, ymax=cih), color="gray", width=0.07, show.legend=FALSE) +
        ...
        geom_text_repel(data=subset(data, VAR=="HAND"), aes(label="hand"), point.padding=unit(0.1, "in"), arrow=arrow(length=unit(0.05, "in")), show.legend=FALSE) +
...
#         geom_errorbar(aes(ymin=cil, ymax=cih), color="gray", width=0.05, show.legend=FALSE) +
#         geom_point(show.legend=FALSE) +        
#         geom_abline(intercept=0, slope=1) +
#         geom_text_repel(data=subset(data, VAR=="OPT"), nudge_x=-0.3, nudge_y=0.17, aes(label="OPT"), point.padding=unit(0.1, "in"), arrow=arrow(length=unit(0.05, "in")), show.legend=FALSE) +
#         geom_text_repel(data=subset(data, VAR=="HAND"), nudge_x=-0.12, nudge_y=0.25, aes(label="HAND"), point.padding=unit(0.1, "in"), arrow=arrow(length=unit(0.05, "in")), show.legend=FALSE) +
#         geom_text_repel(data=subset(data, VAR=="MIN"), nudge_x=-0.16, nudge_y=0.16, aes(label="MIN"), point.padding=unit(0.1, "in"), arrow=arrow(length=unit(0.05, "in")), show.legend=FALSE) +
#         geom_text_repel(data=subset(data, VAR=="MAX"), nudge_x=0.4, nudge_y=0.0, aes(label="MAX"), point.padding=unit(0.1, "in"), arrow=arrow(length=unit(0.05, "in")), show.legend=FALSE) +
#         geom_text_repel(data=subset(data, VAR=="AUTO"), nudge_x=-0.15, nudge_y=-0.11, aes(label="AUTO"), point.padding=unit(0.1, "in"), arrow=arrow(length=unit(0.05, "in")), show.legend=FALSE) +
#         xlim(0.4, 1.69) + 
#         ylim(0.4, 1.69) + 
#         xlab("estimated time [ms]") + 
#         ylab("measured time [ms]") +
